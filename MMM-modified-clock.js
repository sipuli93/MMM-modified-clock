...
/* Magic Mirror
 * Module: MMM-modified-clock
 *
 * Created by Michael Teeuw http://michaelteeuw.nl
 * MIT Licensed.
 *
 * Modified by Sipuli93
 */
Module.register("MMM-modified-clock",{
	...
	// Define styles.
	getStyles: function() {
		return ["MMM-modified-clock_styles.css","weather-icons.css"];
	},
	...
	getDom: function() {
		...
		if (this.config.showSunTimes) {
			...
			if (this.config.showDaylightRemaining) {
				const untilNextEvent = moment.duration(moment(nextEvent).diff(now));

				var untilNextEventSpan = document.createElement("SPAN");
				var untilNextEventIcon = document.createElement("I");
				var untilNextEventString = document.createTextNode(untilNextEvent.hours() + "h " + untilNextEvent.minutes() + "m");
				untilNextEventSpan.className = (isVisible ? "bright" : "");
				untilNextEventIcon.className = "fa fa-sun-o";
				untilNextEventSpan.appendChild(untilNextEventIcon);
				untilNextEventSpan.appendChild(untilNextEventString);
				sunWrapper.appendChild(untilNextEventSpan);
			}

			var sunriseSpan = document.createElement("SPAN");
			var sunriseIcon = document.createElement("I");
			var sunriseTime = document.createTextNode(formatTime(this.config, sunTimes.sunrise));
			sunriseIcon.className = "wi wi-sunrise";
			sunriseSpan.appendChild(sunriseIcon);
			sunriseSpan.appendChild(sunriseTime);
			sunWrapper.appendChild(sunriseSpan);

			var sunsetSpan = document.createElement("SPAN");
			var sunsetIcon = document.createElement("I");
			var sunsetTime = document.createTextNode(formatTime(this.config, sunTimes.sunset));
			sunsetIcon.className = "wi wi-sunset";
			sunsetSpan.appendChild(sunsetIcon);
			sunsetSpan.appendChild(sunsetTime);
			sunWrapper.appendChild(sunsetSpan);
